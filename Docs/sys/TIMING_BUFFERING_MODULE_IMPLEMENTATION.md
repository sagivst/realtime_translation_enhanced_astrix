...
## Extensions 7777/8888 - Bidirectional Latency Compensation
...
3. **Compares bidirectional latencies** (7777→8888 vs 8888→7777)
...
**Problem:** Traditional frame-based latency tracking (e.g., "Frame #123 took 500ms from 7777→8888") is **impossible** because:
...
│  │                                                             │  │
│  │  • ExtensionPairManager (7777↔8888 tracking)              │  │
...
Total E2E = max(serial_total, parallel_total)
    ↓
Compare: e2e_7777_to_8888 vs e2e_8888_to_7777
    ↓
Buffer faster channel by difference
    ↓
Send to Extension 8888 (French audio)
```
...
const avg_7777_to_8888 = getAverageLatency('7777→8888'); // e.g., 500ms
const avg_8888_to_7777 = getAverageLatency('8888→7777'); // e.g., 300ms
const latencyDifference = avg_7777_to_8888 - avg_8888_to_7777; // +200ms

// Interpret the difference:
// +200ms: 7777→8888 is SLOWER, need to buffer 8888→7777 by 200ms
// -200ms: 8888→7777 is SLOWER, need to buffer 7777→8888 by 200ms
```
...
// Example:
// 7777→8888 = 500ms (slower)
// 8888→7777 = 300ms (faster)
// Difference for 7777→8888 = 500 - 300 = +200ms → buffer = max(0, -200) = 0ms
// Difference for 8888→7777 = 300 - 500 = -200ms → buffer = max(0, -(-200)) = 200ms
// Result: Delay 8888→7777 by 200ms to match 7777→8888
```
...
async function processGatewayAudio(socket, extension, audioBuffer, language) {
  try {
    ...
    // ═══════════════════════════════════════════════════════════════
    const targetLang = extension === '7777' ? 'fr' : 'en';
...
Create `/home/azureuser/translation-app/7777-8888-stack/tests/timing-module.test.js`:
...
describe('ExtensionPairManager', () => {
  test('should register and retrieve pairs', () => {
    ...
    manager.registerPair('7777', '8888');

    expect(manager.isPaired('7777')).toBe(true);
    expect(manager.getPairedExtension('7777')).toBe('8888');
    expect(manager.getPairedExtension('8888')).toBe('7777');
  });

  test('should unregister pairs', () => {
    const manager = new ExtensionPairManager();
    manager.registerPair('7777', '8888');
    manager.unregisterPair('7777');

    expect(manager.isPaired('7777')).toBe(false);
    expect(manager.isPaired('8888')).toBe(false);
  });
});
...
describe('LatencyTracker', () => {
  test('should calculate rolling average', () => {
    ...
    for (let i = 1; i <= 10; i++) {
      tracker.updateLatency('7777→8888', i * 100);
    }

    const avg = tracker.getAverageLatency('7777→8888');
  ...
  test('should calculate latency difference', () => {
    ...
    tracker.updateLatency('7777→8888', 500);
    tracker.updateLatency('8888→7777', 300);

    const diff = tracker.getLatencyDifference('7777', '8888');
  ...
  test('should keep only last 10 samples', () => {
    ...
    const samples = tracker.directionSamples.get('7777→8888');
...
describe('AudioBufferManager', () => {
  test('should send immediately when delay is 0', (done) => {
    ...
    manager.bufferAndSend('7777', audio, 0, (ext, data) => {
      expect(ext).toBe('7777');
  ...
  test('should delay audio send', (done) => {
    ...
    manager.bufferAndSend('7777', audio, 100, (ext, data) => {
...
describe('Pipeline Timing Integration', () => {
  ...
  test('should apply correct buffer delay', () => {
    // 7777→8888 = 500ms
    // 8888→7777 = 300ms
    // Expect 8888→7777 to be delayed by 200ms
  });
});
```
...
```bash
echo '{"type":"SUBSCRIBE","extension":"7777"}' | nc localhost 6001
```

4. **Make test call:**
- Dial into extension 7777
- Speak English
- Verify French audio arrives at 8888
- Check console for timing logs

5. **Verify dashboard:**
- Open dashboard at `http://20.170.155.53:3002/dashboard-single.html?ext=7777`
...
2. **Monitor logs:**
```bash
tail -f /home/azureuser/translation-app/7777-8888-stack/translation-server.log | grep -E '(Latency|Buffer|Pipeline|TCP API)'
```

3. **Test with live calls:**
- Make test calls between 7777 and 8888
...
│   └── emitLatencyUpdate()
│
├── gateway-7777-8888.js (unchanged)
...
1. `/home/azureuser/translation-app/7777-8888-stack/conference-server-externalmedia.js`
2. `/home/azureuser/translation-app/7777-8888-stack/bidirectional-timing-server.js`
3. `/home/azureuser/translation-app/7777-8888-stack/public/dashboard-single.html`
