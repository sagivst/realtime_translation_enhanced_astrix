...
export class AudioWriter {
  ...
  enqueueSegment(segment) {
    console.log(`[AudioWriter.enqueueSegment] Received segment: trace=${segment?.meta?.trace_id}, tap=${segment?.meta?.tap}, queue_size=${this.queue.length}`);
  ...
  _writeWavSegment(meta, pcm) {
    ...
    console.log(`[AudioWriter._writeWavSegment] Writing file: ${filePath}`);
