...
const HumeStreamingClient = require('./services/hume-streaming-client');

// Import HMLCP modules
// DISABLED: const { UserProfile, ULOLayer, PatternExtractor } = require('./hmlcp');
// DISABLED: const { applyDefaultProfile } = require('./hmlcp/default-profiles');
// HMLCP Stubs - Replace disabled imports with minimal implementations
class UserProfile {
...
// [DISABLED FOR 5555/6666] // Start AudioSocket server (for Asterisk integration on port 5050)
// [DISABLED FOR 5555/6666] // IMPORTANT: Must load AFTER global.io is set
// [DISABLED FOR 5555/6666] require("./audiosocket-integration");
...
// Extension 5555: English → French (DEFAULT)
...
function getQaConfig(extension) {
  return global.qaConfigs.get(extension) || global.qaConfigs.get('5555');
}
...
async function processGatewayAudio(socket, extension, audioBuffer, language) {
  try {
    ...
    // Step 2: Translate
    const targetLang = extension === '5555' ? 'fr' : 'en'; // 5555 is English->French, 6666 is French->English
    ...
    const transEnd = Date.now();
    const translationDuration = transEnd - transStart;
    console.log(`[Pipeline] Translation: "${translation}" (${translationDuration}ms)`);
    ...
    console.log(`[Pipeline] TTS generated: ${ttsAudio ? ttsAudio.length : 0} bytes (${ttsDuration}ms)`);
...
io.on('connection', (socket) => {
  ...
  // Handle events from ExternalMedia Gateway (extensions 5555/6666)
  // ========================================
...
// ============================================================================
// ============================================================================
// UDP GATEWAY INPUT WITH STREAMING ASR
// Connects gateway PCM to streaming Deepgram pipeline
// ============================================================================

// Using official Deepgram SDK instead of custom client
const { DeepLIncrementalMT } = require('./services/deepl-incremental-mt');

// Initialize UDP Gateway Input

// ==========================================
// UDP Gateway - Direct dgram (like conf-server)
// ==========================================
...
// Buffers for 160->640 byte accumulation
let buffer5555 = Buffer.alloc(0);
let buffer6666 = Buffer.alloc(0);
...
// Send to gateway functions
function sendTo5555(audioBuffer) {
  socket5555Out.send(audioBuffer, UDP_CONFIG.port5555Out, UDP_CONFIG.gatewayHost, (err) => {
    if (!err) udpStats.to5555++;
  });
}

function sendTo6666(audioBuffer) {
  socket6666Out.send(audioBuffer, UDP_CONFIG.port6666Out, UDP_CONFIG.gatewayHost, (err) => {
    if (!err) udpStats.to6666++;
  });
}

// Extension language configuration
const extensionConfig = {
  '5555': { sourceLang: 'en', targetLang: 'fr', targetExt: '6666' },
  '6666': { sourceLang: 'fr', targetLang: 'en', targetExt: '5555' }
};

// Streaming ASR clients for each extension
const streamingClients = {};

// Initialize streaming client for an extension
async function initStreamingClient(extension) {
  ...
  connection.on('Results', async (data) => {
    ...
    if (transcript && transcript.trim()) {
      if (isFinal) {
        console.log(`[UDP-Stream] ${extension} ASR Final: "${transcript}"`);
        ...
        try {
          // Translate
          const deeplTranslator = new DeepLIncrementalMT(process.env.DEEPL_API_KEY);
          const translatedText = await deeplTranslator.translate(
            extension, 
            config.sourceLang, 
            config.targetLang, 
            transcript, 
          ...
          console.log(`[UDP-Stream] ${extension} Translation: "${translatedText}"`);
          ...
          // TTS
          const ttsAudio = await elevenLabsTTS.synthesize(
            translatedText, 
            process.env.ELEVENLABS_DEFAULT_VOICE_ID || 'ErXwobaYiN019PkySvjV'
          );
          
          if (ttsAudio && ttsAudio.length > 0) {
            console.log(`[UDP-Stream] ${extension} TTS: ${ttsAudio.length} bytes`);
            
            // Send to target extension
            if (config.targetExt === '5555') {
              sendAudioInChunks(ttsAudio, (chunk) => sendTo5555(chunk));
            } else {
              sendAudioInChunks(ttsAudio, (chunk) => sendTo6666(chunk));
            }
          }
        } catch (error) {
          console.error(`[UDP-Stream] ${extension} Error: ${error.message}`);
        }
      } else {
        // Partial result
        global.io.emit('transcriptionPartial', {
  ...
  connection.on('Open', () => {
    console.log(`[UDP-Stream] ${extension} Deepgram connected (SDK)`);
  });
  
  connection.on('Error', (error) => {
    console.error(`[UDP-Stream] ${extension} Deepgram error: ${error.message}`);
  });
  
  connection.on('Close', () => {
    console.log(`[UDP-Stream] ${extension} Deepgram closed`);
  });
  
  // Return connection immediately (like 7777/8888 conference-server)
  // SDK will handle connection lifecycle
  return connection;
}

// Send audio in chunks
function sendAudioInChunks(audioBuffer, sendFn) {
  const frameSize = 640;
  let offset = 0;
...
// Socket message handlers (identical to conf-server)
socket5555In.on('message', (msg, rinfo) => {
  udpStats.from5555++;
  
  if (udpStats.from5555 <= 3) {
    console.log(`[UDP] Received from 5555: ${msg.length} bytes`);
  }
  
  // Buffer 160->640 bytes for Deepgram
  buffer5555 = Buffer.concat([buffer5555, msg]);
  
  while (buffer5555.length >= 640) {
    const frame = buffer5555.slice(0, 640);
    buffer5555 = buffer5555.slice(640);
    
    // Send to Deepgram - create connection on demand if needed
    (async () => {
      if (!streamingClients['5555']) {
        console.log('[UDP] Creating Deepgram connection for 5555...');
        streamingClients['5555'] = await initStreamingClient('5555');
      }
      if (streamingClients['5555']) {
        try {
          streamingClients['5555'].send(frame);
        } catch (err) {
          console.error('[UDP] 5555 send error:', err.message);
          // Reset connection for retry
          streamingClients['5555'] = null;
        }
      }
    })();
  }
...
socket5555In.on('listening', () => {
  console.log(`[UDP] ✓ Listening for 5555 on port ${UDP_CONFIG.port5555In}`);
});

socket5555In.on('error', (err) => {
  console.error(`[UDP] ERROR 5555: ${err.message}`);
});

socket6666In.on('message', (msg, rinfo) => {
  udpStats.from6666++;
  
  if (udpStats.from6666 <= 3) {
    console.log(`[UDP] Received from 6666: ${msg.length} bytes`);
  }
  
  // Buffer 160->640 bytes for Deepgram
  buffer6666 = Buffer.concat([buffer6666, msg]);
  
  while (buffer6666.length >= 640) {
    const frame = buffer6666.slice(0, 640);
    buffer6666 = buffer6666.slice(640);
    
    // Send to Deepgram - create connection on demand if needed
    (async () => {
      if (!streamingClients['6666']) {
        console.log('[UDP] Creating Deepgram connection for 6666...');
        streamingClients['6666'] = await initStreamingClient('6666');
      }
      if (streamingClients['6666']) {
        try {
          streamingClients['6666'].send(frame);
        } catch (err) {
          console.error('[UDP] 6666 send error:', err.message);
          // Reset connection for retry
          streamingClients['6666'] = null;
        }
      }
    })();
  }
...
socket6666In.on('listening', () => {
  console.log(`[UDP] ✓ Listening for 6666 on port ${UDP_CONFIG.port6666In}`);
});

socket6666In.on('error', (err) => {
  console.error(`[UDP] ERROR 6666: ${err.message}`);
});

// Initialize Deepgram clients and bind UDP sockets
setTimeout(() => {
  console.log('[UDP-Stream] Initializing UDP listeners...');
  
  try {
    // Bind UDP sockets - Deepgram connections created on demand
    socket5555In.bind(UDP_CONFIG.port5555In);
    socket6666In.bind(UDP_CONFIG.port6666In);
    
    console.log('[UDP-Stream] ✓ UDP sockets bound - Deepgram on-demand');
  } catch (err) {
    console.error('[UDP-Stream] Init error:', err.message);
  }
}, 3000);
