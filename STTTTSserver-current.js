...
// ========================================================================
// STATION KNOB SAFELOADER INTEGRATION
// ========================================================================
...
/**
 * Get current system knob values - these are the ACTUAL values in use
 */
function getCurrentSystemKnobs() {
  // Return actual system values that will be captured as defaults
  return {
    // AGC knobs
    'agc.enabled': true,
    'agc.targetLevel': -18,
...
/**
 * Load station configuration with safe defaults
 */
...
// ========================================================================
// SAFELOADER: Load knob configurations for each station
// ========================================================================
console.log('[SafeLoader] Loading station configurations...');

// Load configurations for all station-extension combinations
const station3_3333_knobs = loadStationConfiguration('STATION_3', '3333');
const station3_4444_knobs = loadStationConfiguration('STATION_3', '4444');
const station4_3333_knobs = loadStationConfiguration('STATION_4', '3333');
const station4_4444_knobs = loadStationConfiguration('STATION_4', '4444');

// Store knob configurations for use in audio processing
global.stationKnobs = {
  'STATION_3-3333': station3_3333_knobs,
  'STATION_3-4444': station3_4444_knobs,
  'STATION_4-3333': station4_3333_knobs,
  'STATION_4-4444': station4_4444_knobs
};

console.log('[SafeLoader] ✓ Station knob configurations loaded and stored');
...
// Add this code after line 4330 in STTTTSserver.js

// Update monitoring agents with real data every second
setInterval(() => {
  try {
    // Update all 4 station agents with real metrics
    // These agents automatically collect ALL 75 metrics and 113 knobs
    
    // Station 3 - 3333 (Voice Monitor/Enhancer for caller)
    if (typeof station3_3333 !== 'undefined' && station3_3333.update) {
      station3_3333.update();
    }
    
    // Station 3 - 4444 (Voice Monitor/Enhancer for callee)
    if (typeof station3_4444 !== 'undefined' && station3_4444.update) {
      station3_4444.update();
    }
    
    // Station 4 - 3333 (Deepgram STT for caller)
    if (typeof station4_3333 !== 'undefined' && station4_3333.update) {
      station4_3333.update();
    }
    
    // Station 4 - 4444 (Deepgram STT for callee)
    if (typeof station4_4444 !== 'undefined' && station4_4444.update) {
      station4_4444.update();
    }
    
  } catch (error) {
    console.error("[Monitoring] Error updating station agents:", error.message);
  }
}, 1000); // Update every second

console.log("[Monitoring] Started real-time monitoring updates for all 4 stations");
}
