#!/usr/bin/env node
/**
 * Gateway for Extension 4444 - BUFFERED VERSION
...
const CONFIG = {
  extensionId: '4444',
  fromAsteriskPort: 4022,
  toAsteriskPort: 4023,
  toConfServerPort: 6122,
  fromConfServerPort: 6123,
  confServerHost: '127.0.0.1',
  sampleRate: 16000,
  frameSizeMs: 20,
  samplesPerFrame: 320,
  maxBufferedPackets: 1000,  // Buffer up to 20 seconds (1000 * 20ms)
  logFile: '/tmp/gateway-4444-buffered.log'
};

const logStream = fs.createWriteStream(CONFIG.logFile, { flags: 'a' });
function log(message) {
  const timestamp = new Date().toISOString();
  const logMessage = `[${timestamp}] [Gateway-4444] ${message}\n`;
...
function init() {
  log('='.repeat(80));
  log('Gateway-4444 - BUFFERED VERSION (Prevents Packet Loss)');
  log('='.repeat(80));
  ...
  log('Gateway-4444 initialized and ready');
}
