#!/usr/bin/env node
/**
 * Test Audio Server - Plays 44.1kHz audio file to extension 8888
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
      try {
        ...
        // When we receive audio from gateway (extension 7777 or 8888)
        if (data.event === 'audio' && data.extension === '8888') {
          ...
          console.log('[8888] Sending test audio @ 44.1kHz...');
          ...
          console.log(`[8888] ✓ Sent ${audioData.length} bytes (${(audioData.length / 88200).toFixed(2)}s)`);
        }
      } catch (e) {
