...
async function processGatewayAudio(socket, extension, audioBuffer, language) {
  try {
    const startTime = Date.now(); // Track total pipeline start time
    ...
    console.log(`[Latency] STT: ${sttDuration}ms, Translation: ${translationDuration}ms, TTS: ${ttsDuration}ms, Total: ${totalLatency}ms`);
    global.io.emit('translated-audio', {
      ...
      targetLang: targetLang,
      timestamp: Date.now(),
      latency: totalLatency, // Total end-to-end latency for dashboard
      timing: {
        stt: sttDuration,
