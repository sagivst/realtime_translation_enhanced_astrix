 ...
 *
 * Architecture:
 *   Phone 1001 → Asterisk → ExternalMedia 9007 → Unified Gateway
 *                                                   ↕ (simple passthrough, no translation)
 *   Phone 1002 → Asterisk → ExternalMedia 9008 → Unified Gateway
 *                                                   ↓ WebSocket (port 3002)
 *                                               Web Browser (listen to audio)
 *
 * What This Tests:
 *   ✅ RTP Reception from Asterisk ExternalMedia (9007/9008)
 *   ✅ RTP Transmission back to Asterisk ExternalMedia
 *   ✅ Audio Routing: Connect 9007 ↔ 9008 inside gateway (simple passthrough)
...
// Statistics
const stats = {
  '9007': { packetsReceived: 0, bytesReceived: 0, packetsSent: 0, bytesSent: 0 },
  '9008': { packetsReceived: 0, bytesReceived: 0, packetsSent: 0, bytesSent: 0 }
};
 ...
 * @param {string} extension - Extension number (9007/9008)
 ...
 * @param {string} extension - Target extension (9007/9008)
 * @param {Buffer} audioPayload - PCM16 audio data
 */
 ...
 * @param {string} extension - Source extension (9007/9008)
...
function processAudioChunk(extension, audioChunk) {
  ...
  const targetExtension = extension === '9007' ? '9008' : '9007';
...
async function connectARI() {
  try {
    ...
    ariClient.on('StasisStart', async (event, channel) => {
      ...
      // Check if this is an incoming call to 9007 or 9008
      const args = event.args;
      if (!args || args.length === 0) return;

      const extension = args[0].replace('ext', ''); // 'ext9007' -> '9007'
...
async function main() {
  ...
  console.log('  Phone 1001 → Asterisk → ExternalMedia 9007 → Gateway');
  console.log('                                                  ↕ (passthrough)');
  console.log('  Phone 1002 → Asterisk → ExternalMedia 9008 → Gateway');
  ...
  await createExternalMediaChannel('9007');
  await new Promise(resolve => setTimeout(resolve, 1000));
  await createExternalMediaChannel('9008');
  ...
  console.log('1. Call 1001 → 9007: Audio goes to gateway');
  console.log('2. Call 1002 → 9008: Audio goes to gateway');
  console.log('3. Gateway connects them: 9007 audio → 9008, and 9008 audio → 9007');
  ...
  // Stats reporter
  setInterval(() => {
    if (connectedClients > 0 || stats['9007'].packetsReceived > 0 || stats['9008'].packetsReceived > 0) {
      const summary = {
        clients: connectedClients,
        '9007': {
          RX: `${stats['9007'].packetsReceived} pkts (${(stats['9007'].bytesReceived / 1024).toFixed(2)} KB)`,
          TX: `${stats['9007'].packetsSent} pkts (${(stats['9007'].bytesSent / 1024).toFixed(2)} KB)`
        },
        '9008': {
          RX: `${stats['9008'].packetsReceived} pkts (${(stats['9008'].bytesReceived / 1024).toFixed(2)} KB)`,
          TX: `${stats['9008'].packetsSent} pkts (${(stats['9008'].bytesSent / 1024).toFixed(2)} KB)`
        }
      };
