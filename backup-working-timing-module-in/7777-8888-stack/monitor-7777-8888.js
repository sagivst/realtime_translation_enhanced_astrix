...
// Track endpoints
let endpoint7777 = null;
let endpoint8888 = null;
...
// Stats
const stats = {
  '7777': { packets: 0, bytes: 0, lastUpdate: Date.now(), bytesPerSec: 0 },
  '8888': { packets: 0, bytes: 0, lastUpdate: Date.now(), bytesPerSec: 0 }
};
...
<body>
  <div class="container">
    <h1>Audio Monitor - Extensions 7777 & 8888</h1>
    ...
    <div class="controls">
      <div class="channel">
        <h2>Extension 7777</h2>

        <div class="control-group">
          <div class="label">
            <span>Microphone</span>
            <span class="value" id="vol-7777-mic">100%</span>
          </div>
          <input type="range" id="slider-7777-mic" min="0" max="100" value="100">
          <div class="meter">
            <div class="meter-bar" id="meter-7777-mic"></div>
            <div class="meter-text" id="level-7777-mic">0</div>
          </div>
        </div>
        ...
        <div class="control-group">
          <div class="label">
            <span>Speaker</span>
            <span class="value" id="vol-7777-speaker">100%</span>
          </div>
          <input type="range" id="slider-7777-speaker" min="0" max="100" value="100">
          <div class="meter">
            <div class="meter-bar" id="meter-7777-speaker"></div>
            <div class="meter-text" id="level-7777-speaker">0</div>
          </div>
        </div>
      </div>

      <div class="channel">
        <h2>Extension 8888</h2>

        <div class="control-group">
          <div class="label">
            <span>Microphone</span>
            <span class="value" id="vol-8888-mic">100%</span>
          </div>
          <input type="range" id="slider-8888-mic" min="0" max="100" value="100">
          <div class="meter">
            <div class="meter-bar" id="meter-8888-mic"></div>
            <div class="meter-text" id="level-8888-mic">0</div>
          </div>
        </div>
        ...
        <div class="control-group">
          <div class="label">
            <span>Speaker</span>
            <span class="value" id="vol-8888-speaker">100%</span>
          </div>
          <input type="range" id="slider-8888-speaker" min="0" max="100" value="100">
          <div class="meter">
            <div class="meter-bar" id="meter-8888-speaker"></div>
            <div class="meter-text" id="level-8888-speaker">0</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    ['7777-mic', '7777-speaker', '8888-mic', '8888-speaker'].forEach(channel => {
...
// UDP 5000 - Extension 7777
socket5000.on('message', (msg, rinfo) => {
  if (!endpoint7777 || endpoint7777.port !== rinfo.port) {
    endpoint7777 = { address: rinfo.address, port: rinfo.port };
    console.log(`[7777] Endpoint: ${rinfo.address}:${rinfo.port}`);
  }

  stats['7777'].packets++;
  stats['7777'].bytes += msg.length;

  const micLevel = calculateAudioLevel(msg);
  broadcastAudioLevel('7777-mic', micLevel);

  // Extract PCM payload and broadcast for playback
  if (msg.length >= 12) {
    const pcmPayload = msg.slice(12);
    broadcastAudioData('7777-mic', pcmPayload);
  }

  if (endpoint8888) {
    const scaledMsg = applyVolume(msg, volumes['8888_speaker']);
    const speakerLevel = calculateAudioLevel(scaledMsg);
    broadcastAudioLevel('8888-speaker', speakerLevel);

    // Extract PCM payload and broadcast for playback
    if (scaledMsg.length >= 12) {
      const pcmPayload = scaledMsg.slice(12);
      broadcastAudioData('8888-speaker', pcmPayload);
    }

    // DISABLED TO PREVENT RTP LOOPBACK:     socket5001.send(scaledMsg, endpoint8888.port, endpoint8888.address);
  }
});

// UDP 5001 - Extension 8888
socket5001.on('message', (msg, rinfo) => {
  if (!endpoint8888 || endpoint8888.port !== rinfo.port) {
    endpoint8888 = { address: rinfo.address, port: rinfo.port };
    console.log(`[8888] Endpoint: ${rinfo.address}:${rinfo.port}`);
  }

  stats['8888'].packets++;
  stats['8888'].bytes += msg.length;

  const micLevel = calculateAudioLevel(msg);
  broadcastAudioLevel('8888-mic', micLevel);

  // Extract PCM payload and broadcast for playback
  if (msg.length >= 12) {
    const pcmPayload = msg.slice(12);
    broadcastAudioData('8888-mic', pcmPayload);
  }

  if (endpoint7777) {
    const scaledMsg = applyVolume(msg, volumes['7777_speaker']);
    const speakerLevel = calculateAudioLevel(scaledMsg);
    broadcastAudioLevel('7777-speaker', speakerLevel);

    // Extract PCM payload and broadcast for playback
    if (scaledMsg.length >= 12) {
      const pcmPayload = scaledMsg.slice(12);
      broadcastAudioData('7777-speaker', pcmPayload);
    }

    // DISABLED TO PREVENT RTP LOOPBACK:     socket5000.send(scaledMsg, endpoint7777.port, endpoint7777.address);
  }
});

socket5000.bind(6000, '127.0.0.1', () => {
  console.log('Listening on UDP 127.0.0.1:6000 (Extension 7777)');
});

socket5001.bind(6001, '127.0.0.1', () => {
  console.log('Listening on UDP 127.0.0.1:6001 (Extension 8888)');
});
