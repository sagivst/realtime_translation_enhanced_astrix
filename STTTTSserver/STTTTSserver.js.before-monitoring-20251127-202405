...
global.qaConfigs.set('3333', { sourceLang: 'en', targetLang: 'en', qaMode: true });
global.qaConfigs.set('4444', { sourceLang: 'en', targetLang: 'en', qaMode: true });
...
async function translateText(text, sourceLang, targetLang) {
  console.log(`[DEBUG-FIX5 translateText] ENTRY: sourceLang="${sourceLang}", targetLang="${targetLang}", text="${text.substring(0, 50)}..."`);
  if (!translator) {
  ...
  console.log(`[DEBUG-FIX5 translateText] Bypass check: sourceLang="${sourceLang}", targetLang="${targetLang}", equal=${sourceLang === targetLang}`);
...
async function processGatewayAudio(socket, extension, audioBuffer, language) {
  try {
    ...
    console.log(`[Pipeline] Translating ${sourceLang} -> ${targetLang}: "${transcription}"`);
    console.log(`[DEBUG-FIX5] Extension ${extension} - Full config:`, JSON.stringify(config));    console.log(`[DEBUG-FIX5] Extension ${extension} - sourceLang="${sourceLang}", targetLang="${targetLang}", qaMode=${config.qaMode}`);

    // ═══════════════════════════════════════════════════════════════
    // STAGE 4: MT Processing (DeepL)
    // ═══════════════════════════════════════════════════════════════
    const mtStart = Date.now();
    const translation = await translateText(transcription, sourceLang, targetLang);
...
socket3333In.on('message', async (msg, rinfo) => {
  ...
  if (buffer.totalBytes >= UDP_PCM_CONFIG.bufferThreshold) {
    ...
    const config3333 = getQaConfig('3333');
    await processGatewayAudio(null, '3333', combinedBuffer, config3333.sourceLang);
  }
});
...
socket4444In.on('message', async (msg, rinfo) => {
  ...
  if (buffer.totalBytes >= UDP_PCM_CONFIG.bufferThreshold) {
    ...
    const config4444 = getQaConfig('4444');
    await processGatewayAudio(null, '4444', combinedBuffer, config4444.sourceLang);
  }
});
