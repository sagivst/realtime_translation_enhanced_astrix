...
const RTP_CONFIG = {
  ...
  48000: {
    payloadType: 10,           // PT=10 (hack: using it for 48kHz)
    format: "L16",
    sampleRate: 48000,
    bytesPerSample: 2,
    endianness: "big",         // PT=10 requires big-endian
...
const INCOMING_RTP_CONFIG = RTP_CONFIG[AUDIO_SAMPLE_RATE];  // Match ExternalMedia format (slin48 = 48kHz)
const OUTGOING_RTP_CONFIG = RTP_CONFIG[AUDIO_SAMPLE_RATE];  // Use PT=10 but send 48kHz data
...
function forwardTranslatedAudioToAsterisk(extension, audioData) {
  ...
  // 20ms chunk size depends on sample rate: sampleRate/50 samples * 2 bytes
  const CHUNK_SIZE = (AUDIO_SAMPLE_RATE / 50) * 2; // 16kHz=640, 48kHz=1920
  const chunks = [];
