# 7777-8888 ExternalMedia Translation System Documentation
...
The 7777-8888 system is a real-time translation platform that:
- Receives phone calls on Asterisk extensions 7777 (English) and 8888 (French)
...
**Root Directory:** `/home/azureuser/translation-app/7777-8888-stack/`

```
7777-8888-stack/
├── gateway-7777-8888.js          # MAIN GATEWAY - UDP to Socket.IO bridge
...
### 1. gateway-7777-8888.js (MAIN GATEWAY)
...
const EXTERNAL_MEDIA_CONFIG = {
  '7777': { udpPort: 5000, language: 'en' },
  '8888': { udpPort: 5001, language: 'fr' }
};
```
...
|------|---------|-------------|
| 5000 | gateway-7777-8888.js | RTP audio from Asterisk (ext 7777) |
| 5001 | gateway-7777-8888.js | RTP audio from Asterisk (ext 8888) |
...
```bash
node gateway-7777-8888.js > /tmp/gateway-7777-8888.log 2>&1 &
```
**Wait for:** "Gateway is READY"

### Verification
```bash
pgrep -af 'conference-server-externalmedia|gateway-7777'
...
```bash
pgrep -af gateway-7777-8888
ss -ulnp | grep -E '5000|5001'
tail -50 /tmp/gateway-7777-8888.log
```
**Solution:** Use gateway-7777-8888.js (NOT ari-externalmedia-handler.js)
...
- **ALWAYS USE:** gateway-7777-8888.js for full translation pipeline
