...
# Gateway-3333 GStreamer Pipeline with proper 8kHz↔16kHz conversion
...
pkill -f "udpsrc port=6121"
sleep 1

# Pipeline 1: RTP from Asterisk (4000) @ 8kHz -> PCM to STTTSserver (6120) @ 16kHz
echo "[Gateway-3333] Starting RTP(8kHz)->PCM(16kHz) pipeline (4000->6120)..."
gst-launch-1.0 -v \
...
RTP2PCM_PID=$!
echo "[Gateway-3333] RTP(8kHz)->PCM(16kHz) pipeline started (PID: $RTP2PCM_PID)"

# Pipeline 2: PCM from STTTSserver (6121) @ 16kHz -> RTP to Asterisk (4001) @ 8kHz
echo "[Gateway-3333] Starting PCM(16kHz)->RTP(8kHz) pipeline (6121->4001)..."
gst-launch-1.0 -v \
    udpsrc port=6121 ! \
...
PCM2RTP_PID=$!
echo "[Gateway-3333] PCM(16kHz)->RTP(8kHz) pipeline started (PID: $PCM2RTP_PID)"

echo ""
echo "[Gateway-3333] Both pipelines running with proper resampling:"
echo "  - RTP→PCM: PID $RTP2PCM_PID (8kHz from Asterisk → 16kHz to AI)"
echo "  - PCM→RTP: PID $PCM2RTP_PID (16kHz from AI → 8kHz to Asterisk)"
echo ""
echo "[Gateway-3333] Logs:"
echo "  - /tmp/gstreamer-3333-rtp2pcm.log"
echo "  - /tmp/gstreamer-3333-pcm2rtp.log"
echo ""
echo "[Gateway-3333] Ready for calls to extension 3333"
echo "[Gateway-3333] Audio flow: Asterisk(8kHz) → Upsample → AI(16kHz) → Downsample → Asterisk(8kHz)"
...
while true; do
    if ! kill -0 $RTP2PCM_PID 2>/dev/null; then
        echo "[Gateway-3333] WARNING: RTP->PCM pipeline died, restarting..."
        gst-launch-1.0 -v \
            udpsrc port=4000 caps="application/x-rtp,media=audio,clock-rate=8000,encoding-name=PCMA,channels=1,payload=8" ! \
            rtppcmadepay ! \
            alawdec ! \
    ...
    if ! kill -0 $PCM2RTP_PID 2>/dev/null; then
        echo "[Gateway-3333] WARNING: PCM->RTP pipeline died, restarting..."
        gst-launch-1.0 -v \
            udpsrc port=6121 ! \
            rawaudioparse use-sink-caps=false format=pcm pcm-format=s16le sample-rate=16000 num-channels=1 ! \
            audioconvert ! \
