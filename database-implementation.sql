...
CREATE TABLE calls (
    ...
    INDEX idx_calls_external (external_call_id)
);
...
CREATE TABLE channels (
    ...
    leg VARCHAR(10),
    metadata JSONB,
    INDEX idx_channels_call (call_id)
);
...
CREATE TABLE segments (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_segments_channel (channel_id),
    INDEX idx_segments_time (start_ms, end_ms)
);
...
CREATE TABLE station_snapshots (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_snapshots_segment (segment_id),
    INDEX idx_snapshots_station (station_id),
...
CREATE TABLE optimizer_runs (
    ...
    error_message TEXT,
    INDEX idx_optimizer_status (status),
    INDEX idx_optimizer_station (station_id)
);
...
CREATE TABLE parameters (
    ...
    max_value JSONB,
    description TEXT,
    INDEX idx_parameters_station (station_id)
);
...
CREATE TABLE parameter_changes (
    ...
    notes TEXT,
    INDEX idx_changes_parameter (parameter_id),
    INDEX idx_changes_applied (applied_at)
);
...
CREATE VIEW latest_station_snapshots AS
...
CREATE VIEW optimization_performance AS
SELECT
    station_id,
    COUNT(*) as total_runs,
...
CREATE USER audio_app WITH PASSWORD 'SecurePass2025!';

-- Grant permissions
GRANT CONNECT ON DATABASE audio_optimization TO audio_app;
GRANT USAGE ON SCHEMA public TO audio_app;
GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA public TO audio_app;
