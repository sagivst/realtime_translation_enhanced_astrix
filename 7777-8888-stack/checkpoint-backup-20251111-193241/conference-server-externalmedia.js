...
class LatencyTracker {
  ...
  getCurrentLatencyDifference(ext1, ext2) {
    ...
    const difference = current1 - current2;
...
// Initialize timing module instances (NOT YET USED - Step 1 only)
...
async function processGatewayAudio(socket, extension, audioBuffer, language) {
  try {
    ...
    // NOTE: In Step 2, we only calculate but DO NOT apply buffer yet
    // ═══════════════════════════════════════════════════════════════
    const pairedExtension = pairManager.getPairedExtension(extension);
    const bufferDelayMs = 0; // Step 2: Don't apply buffer yet, just measure
    ...
    // ═══════════════════════════════════════════════════════════════
    // STEP 4: CALCULATE BUFFER AMOUNT (DON'T APPLY YET)
    // ═══════════════════════════════════════════════════════════════
    // Calculate latency difference using CURRENT E2E values (not averages)
    // IMPORTANT: positive = this ext is SLOWER (higher latency), negative = this ext is FASTER (lower latency)
    const latencyDifference = latencyTracker.getCurrentLatencyDifference(extension, pairedExtension);

    // Calculate target buffer amount (but don't apply yet - that's STEP 5)
    let targetBufferMs = 0;
    if (latencyDifference === null) {
      // One or both extensions don't have data yet - skip buffer calculation
      targetBufferMs = 0;
      console.log('[Buffer] Skipping buffer calculation - waiting for both extensions to have data');
    } else if (latencyDifference < 0) {
      // This extension is FASTER (lower latency) than its pair, so it should be buffered
      targetBufferMs = Math.abs(latencyDifference);
      console.log('[Buffer] Extension ' + extension + ' is ' + Math.round(Math.abs(latencyDifference)) + 'ms FASTER (lower latency) than ' + pairedExtension);
      console.log('[Buffer] Target buffer calculated: ' + Math.round(targetBufferMs) + 'ms (NOT APPLIED YET - STEP 5)');
    } else if (latencyDifference > 0) {
      // This extension is SLOWER (higher latency), no buffer needed
      targetBufferMs = 0;
      console.log('[Buffer] Extension ' + extension + ' is ' + Math.round(latencyDifference) + 'ms SLOWER (higher latency) than ' + pairedExtension);
      console.log('[Buffer] No buffer needed for slower extension');
    } else {
      // Latencies are equal
      console.log('[Buffer] Extensions ' + extension + ' and ' + pairedExtension + ' have equal latencies');
    }

    // NOTE: bufferDelayMs stays at 0 for STEP 4 - we're only calculating, not applying
    // STEP 5 will actually apply the buffer using audioBufferManager.bufferAndSend()
    ...
    emitLatencyUpdate(extension, timing, bufferDelayMs, targetBufferMs, isParallelBlocking);
...
function emitLatencyUpdate(extension, timing, appliedBufferMs, targetBufferMs, isParallelBlocking) {
  const pairedExtension = pairManager.getPairedExtension(extension);
  ...
  const data = {
    ...
    buffer: {
      ...
      adjustment: targetBufferMs,       // Show target buffer for dashboard (STEP 5 not implemented yet)
  ...
  console.log('[Dashboard] Emitted latency update for ' + extension + ' (E2E: ' + Math.round(timing.e2eTotal) + 'ms)');
}
