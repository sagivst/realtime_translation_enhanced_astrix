#!/usr/bin/env node
/**
 * Gateway for Extension 6666 - BUFFERED VERSION
...
const CONFIG = {
  extensionId: '6666',
  fromAsteriskPort: 4012,
  toAsteriskPort: 4013,
  toConfServerPort: 6112,
  fromConfServerPort: 6113,
  confServerHost: '127.0.0.1',
  sampleRate: 16000,
  frameSizeMs: 20,
  samplesPerFrame: 320,
  maxBufferedPackets: 1000,  // Buffer up to 20 seconds (1000 * 20ms)
  logFile: '/tmp/gateway-6666-buffered.log'
};

const logStream = fs.createWriteStream(CONFIG.logFile, { flags: 'a' });
function log(message) {
  const timestamp = new Date().toISOString();
  const logMessage = `[${timestamp}] [Gateway-6666] ${message}\n`;
...
function shutdown() {
  log('Shutting down gateway-6666...');
  reportStats();
...
function init() {
  log('='.repeat(80));
  log('Gateway-6666 - BUFFERED VERSION (Prevents Packet Loss)');
  log('='.repeat(80));
  ...
  log('Gateway-6666 initialized and ready');
}
