...
export class DatabaseBridge {
  ...
  async sendAggregatedMetrics(batch) {
    ...
    for (const metric of batch) {
      if (metric.trace_id && !uniqueTraces.has(metric.trace_id)) {
        ...
        // Store minimal context for trace creation
  ...
  async saveKnobSnapshot(snapshotData) {
    const query = `
      INSERT INTO knob_snapshots_5s (
        trace_id, station_key, bucket_ts, bucket_ms, knobs_json, config_version
      )
      VALUES ($1, $2, $3, $4, $5, $6)
      ON CONFLICT (trace_id, station_key, bucket_ts)
      DO UPDATE SET knobs_json = EXCLUDED.knobs_json, config_version = EXCLUDED.config_version
    `;
  ...
  /**
   * Get knob history from database
   * @param {string} traceId - Optional trace ID to filter by
   * @param {number} limitHours - How many hours back to look (default 72)
   */
  async getKnobHistory(traceId, limitHours = 72) {
    try {
      let query;
      let params;
      ...
      if (traceId) {
        query = `
          SELECT * FROM knob_events
          WHERE trace_id = $1
          AND occurred_at > NOW() - INTERVAL '${limitHours} hours'
        ...
        query = `
          SELECT * FROM knob_events
          WHERE occurred_at > NOW() - INTERVAL '${limitHours} hours'
          ORDER BY occurred_at DESC
        `;
        params = [];
      }
      ...
      console.error('[DatabaseBridge] Failed to get knob history:', error);
      throw error;
    }
  }
   ...
   * @param {string} stationKey - Optional station key to filter by
   * @param {number} limitHours - How many hours back to look (default 1)
   */
  async getKnobSnapshots(stationKey, limitHours = 1) {
    try {
      let query;
      let params;
      ...
      if (stationKey) {
        query = `
          SELECT * FROM knob_snapshots_5s
          WHERE station_key = $1
          AND bucket_ts > NOW() - INTERVAL '${limitHours} hours'
        ...
        query = `
          SELECT * FROM knob_snapshots_5s
          WHERE bucket_ts > NOW() - INTERVAL '${limitHours} hours'
          ORDER BY bucket_ts DESC
          LIMIT 1000
        `;
        params = [];
      ...
      console.error('[DatabaseBridge] Failed to get knob snapshots:', error);
      throw error;
    }
  }
}
