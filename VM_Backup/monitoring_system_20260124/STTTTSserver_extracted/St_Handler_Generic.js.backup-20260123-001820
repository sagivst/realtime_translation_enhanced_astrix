...
export class St_Handler_Generic {
  ...
  constructor({ config, metricsEmitter, audioRecorder, bucketMs = 5000 }) {
    ...
    // Initialize default compute functions for metrics
    this.defaultMetricComputers = this._getDefaultComputeFunctions();
  ...
  // Default compute functions for missing metrics
  _getDefaultComputeFunctions() {
    return {
      sample_rate_actual: (frame, ctx) => {
        return ctx.sampleRate || 16000;
      },
      bit_depth: (frame, ctx) => {
        return 16;
      },
      channel_count: (frame, ctx) => {
        return ctx.channels || 1;
      },
      consecutive_clipped_frames: (frame, ctx) => {
        let count = 0;
        const maxValue = 32767;
        for (let i = 0; i < frame.length; i++) {
          if (Math.abs(frame[i]) >= maxValue) {
            count++;
          } else {
            break;
          }
      ...
      dropped_frames: (frame, ctx) => {
        return ctx.droppedFrames || 0;
      },
      queue_depth: (frame, ctx) => {
        return ctx.queueSize || 0;
      },
      frozen_signal_detection: (frame, ctx) => {
        if (frame.length < 2) return 0;
        const firstValue = frame[0];
        for (let i = 1; i < frame.length; i++) {
          if (frame[i] !== firstValue) return 0;
        }
        return 1;
      }
    };
    ...
    for (const key of metricKeys) {
      ...
      try {
        ...
        if (typeof def.compute !== "function") {
          if (this.defaultMetricComputers && this.defaultMetricComputers[key]) {
            def = { compute: this.defaultMetricComputers[key] };
          } else {
            console.warn(`No compute function for metric ${key}, skipping`);
            continue;
          }
        }
      ...
      this.aggregator.addSample({
        station_group: ctx.station_group || "STTTTS_PCM",
        layer: tap,  // Use tap as layer (PRE or POST)
        direction: ctx.direction || "RX",
