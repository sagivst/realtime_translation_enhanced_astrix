 ...
 *
 * PARALLEL SYSTEM:
 * - Extensions 9007/9008 use ExternalMedia (ports 5000/5001) + WebSocket Translation
...
const TRANSLATION_SERVER_URL = 'http://localhost:3012';
...
const EXTERNAL_MEDIA_CONFIG = {
  '9007': {
    extension: '9007',
    udpPort: 5010,
    externalHost: '127.0.0.1:5010',  // Send RTP to monitor on port 6000
    language: 'en'  // English
  },
  '9008': {
    extension: '9008',
    udpPort: 5011,
    externalHost: '127.0.0.1:5011',  // Send RTP to monitor on port 6001
    language: 'fr'  // French
  }
};
...
function initializeUDPListeners() {
  Object.values(EXTERNAL_MEDIA_CONFIG).forEach(config => {
    ...
    socket.on('message', (msg, rinfo) => {
      ...
      const monitorPort = ext === '9007' ? 6000 : 6001;
...
async function handleStasisStart(event, client) {
  ...
  // ExternalMedia channels have names like "UnicastRTP/127.0.0.1:5010-..."
...
async function main() {
  ...
  console.log('  - Extension 9007 -> UDP port 5000 (English)');
  console.log('  - Extension 9008 -> UDP port 5001 (French)');
  ...
  console.log('  - Extensions 9007/9008 use ExternalMedia + WebSocket Translation');
  console.log('');
