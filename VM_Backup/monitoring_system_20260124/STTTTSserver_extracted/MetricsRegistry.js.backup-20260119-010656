...
export const MetricsRegistry = {
  ...
  "pcm.lufs_momentary": {
    ...
    realtimeSafe: false,
    compute: (samples, ctx) => {
      // LUFS momentary: 400ms window using persistent buffer
      ...
      const meanSquare = ctx.shortTermBuffer.reduce((sum, val) => sum + val * val, 0) / ctx.shortTermBuffer.length;
      if (meanSquare === 0) return -70;

      const lufs = 20 * Math.log10(Math.sqrt(meanSquare)) + 0.691;
  ...
  "pcm.lufs_short_term": {
    ...
    realtimeSafe: false,
    compute: (samples, ctx) => {
      // LUFS short-term: 3 second window
      if (!ctx.integratedBuffer) ctx.integratedBuffer = [];

      const momentary = ctx.metrics?.['pcm.lufs_momentary'] || -70;
      if (momentary > -70) {
        ctx.integratedBuffer.push(momentary);
        if (ctx.integratedBuffer.length > 150) ctx.integratedBuffer.shift();
      }

      if (ctx.integratedBuffer.length === 0) return -70;
      return ctx.integratedBuffer.reduce((a, b) => a + b, 0) / ctx.integratedBuffer.length;
    }
  },
  ...
  "pcm.lufs_integrated": {
    ...
    realtimeSafe: false,
    compute: (samples, ctx) => {
      // LUFS integrated: entire session
      if (!ctx.loudnessBuffer) ctx.loudnessBuffer = [];

      const shortTerm = ctx.metrics?.['pcm.lufs_short_term'] || -70;
      if (shortTerm > -70) {
        ctx.loudnessBuffer.push(shortTerm);
      }

      const gated = ctx.loudnessBuffer.filter(v => v > -70);
      if (gated.length === 0) return -70;

      return gated.reduce((a, b) => a + b, 0) / gated.length;
    }
  },
  ...
  "pcm.speech_probability": {
    ...
    realtimeSafe: false,
    compute: (samples, ctx) => {
      // Speech probability from VAD history
      if (!ctx.vadHistory) ctx.vadHistory = [];

      const rms = ctx.metrics?.['pcm.rms_dbfs'] || -100;
      const isSpeech = rms > -40;
  ...
  "pcm.speech_segments_per_min": {
    ...
    realtimeSafe: false,
    compute: (samples, ctx) => {
      // Count speech onset transitions per minute
      ...
      const timeSpan = windowSize / 50;
      return (transitions / timeSpan) * 60;
    }
  },
  ...
  "pcm.avg_speech_duration_ms": {
    ...
    compute: (samples, ctx) => {
      ...
      if (ctx.currentSpeechStart === undefined) ctx.currentSpeechStart = null;

      const rms = ctx.metrics?.['pcm.rms_dbfs'] || -100;
      const isSpeech = rms > -40;

      if (isSpeech && !ctx.currentSpeechStart) {
        ctx.currentSpeechStart = Date.now();
      } else if (!isSpeech && ctx.currentSpeechStart) {
  ...
  "pcm.snr_db": {
    ...
    realtimeSafe: false,
    compute: (samples, ctx) => {
      const signalLevel = ctx.metrics?.['pcm.rms_dbfs'] || -60;
  ...
  "pcm.noise_floor_dbfs": {
    ...
    realtimeSafe: false,
    compute: (samples, ctx) => {
      const rms = ctx.metrics?.['pcm.rms_dbfs'] || -60;
      const isSpeech = rms > -40;

      if (!ctx.lastNoiseFloor) ctx.lastNoiseFloor = -60;

      if (!isSpeech && rms < -20 && rms > -80) {
        const alpha = 0.95;
        ctx.lastNoiseFloor = alpha * ctx.lastNoiseFloor + (1 - alpha) * rms;
      }
  ...
  "pcm.background_noise_level": {
    ...
    compute: (samples, ctx) => {
      ...
      if (!ctx.noiseSum) ctx.noiseSum = 0;
      if (!ctx.noiseCount) ctx.noiseCount = 0;
      ...
      return ctx.noiseCount > 0 ? (ctx.noiseSum / ctx.noiseCount) : -60;
    }
  },
