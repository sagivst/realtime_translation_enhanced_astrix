...
// Generate all 75 parameters from base metrics (55 base + 20 DSP)
function generate75Parameters(baseMetrics) {
  ...
  return {
    ...
    // NEW: DSP Enhancement category (20 parameters)
    dsp: {
      hpf: {
...
async function executeCalibrationRun(stationId, duration = 5000) {
  ...
  const interval = setInterval(() => {
    metricsCollected.push(generate75Parameters(station.metrics));
...
io.on('connection', (socket) => {
  ...
  // ═════════════════════════════════════════
  // REAL STATION METRICS FROM STTTTSserver.js
  // ═════════════════════════════════════════
  
  socket.on('stationMetrics', (data) => {
    const { stationId, timestamp, extension, ...metrics } = data;
    console.log(`[StationMetrics] Received from ${stationId} (ext: ${extension}):`, metrics);
    
    // Map station IDs (STATION-3 -> station-3)
    const normalizedStationId = stationId.toLowerCase().replace('station-', 'station-');
    
    // Update station state based on incoming data
    if (stations[normalizedStationId]) {
      const station = stations[normalizedStationId];
      ...
      switch (stationId) {
        case 'STATION-3': // Before Deepgram
          if (metrics.bufferSize) {
            station.metrics.bytesRx = (station.metrics.bytesRx || 0) + metrics.bufferSize;
            station.metrics.packetsRx = (station.metrics.packetsRx || 0) + 1;
          }
          if (metrics.gainApplied) {
            // Store gain for DSP metrics
            station.customMetrics = station.customMetrics || {};
            station.customMetrics.currentGain = metrics.gainApplied;
          }
          break;
        ...
        case 'STATION-4': // After Deepgram
          if (metrics.transcription) {
            station.customMetrics = station.customMetrics || {};
            station.customMetrics.lastTranscription = metrics.transcription;
            station.customMetrics.confidence = metrics.confidence;
          }
          if (metrics.latency) {
            station.metrics.avgLatency = metrics.latency;
          }
          break;
          
        case 'STATION-9': // Before Gateway
          if (metrics.audioBufferSize) {
            station.metrics.bytesTx = (station.metrics.bytesTx || 0) + metrics.audioBufferSize;
            station.metrics.packetsTx = (station.metrics.packetsTx || 0) + 1;
          }
          if (metrics.latencyStages) {
            station.customMetrics = station.customMetrics || {};
            station.customMetrics.latencyStages = metrics.latencyStages;
            // Update average latency from stages
            const stages = metrics.latencyStages;
            const totalLatency = (stages.asr || 0) + (stages.mt || 0) + (stages.tts || 0);
            station.metrics.avgLatency = totalLatency;
          }
          break;
          
        case 'STATION-11': // Before Hume
          if (metrics.chunkSize) {
            station.metrics.bytesRx = (station.metrics.bytesRx || 0) + metrics.chunkSize;
            station.metrics.packetsRx = (station.metrics.packetsRx || 0) + 1;
          }
          if (metrics.humeConnected !== undefined) {
            station.customMetrics = station.customMetrics || {};
            station.customMetrics.humeConnected = metrics.humeConnected;
          }
          break;
      }
      
      // Update last update timestamp
      station.metrics.lastUpdate = timestamp;
      
      // Generate full 75 parameters with updated data
...
app.get('/api/stations/:id/metrics', (req, res) => {
  ...
  const metrics = generate75Parameters(station.metrics);
  res.json({
    stationId,
    timestamp: Date.now(),
    metrics
...
// ═════════════════════════════════════════════════════════════
// CLIENT CONNECTION TO STTTTSserver.js (port 3020)
// Listen for real stationMetrics events from the translation pipeline
// ═════════════════════════════════════════════════════════════

const ioClient = require('socket.io-client');
const sttttServerSocket = ioClient('http://localhost:3020');

sttttServerSocket.on('connect', () => {
  console.log('[STTTTSserver Client] Connected to STTTTSserver.js on port 3020');
});

sttttServerSocket.on('disconnect', () => {
  console.log('[STTTTSserver Client] Disconnected from STTTTSserver.js');
});

sttttServerSocket.on('stationMetrics', (data) => {
  const { stationId, timestamp, extension, ...metrics } = data;
  console.log(`[StationMetrics] Received from ${stationId} (ext: ${extension}):`, metrics);
...
// DISABLED FOR REAL DATA:     station.metrics.lastUpdate = Date.now();
// DISABLED FOR REAL DATA: 
// DISABLED FOR REAL DATA:     // Generate full 75 parameters from base metrics
// DISABLED FOR REAL DATA:     const fullMetrics = generate75Parameters(station.metrics);
// DISABLED FOR REAL DATA: 
// DISABLED FOR REAL DATA:     // Emit update with ALL 75 parameters to all connected clients
