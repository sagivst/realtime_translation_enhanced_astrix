 ...
 * FIXED: RTP sequence number wrapping (0-65535)
 *
...
gstDownsampler.stdout.on('data', (alawData) => {
  ...
  // FIX: Wrap sequence number to 16-bit range (0-65535)
  rtpPacket.writeUInt16BE(rtpState.seq & 0xFFFF, 2);
  rtpState.seq = (rtpState.seq + 1) & 0xFFFF;
...
log('='.repeat(80));
log('Gateway-4444 OPERATIONAL Ready (WITH RTP SESSION RESET FIX + SEQ WRAPPING)');
log('FIX: Detects new SSRC and resets state for consecutive calls');
log('FIX: RTP sequence number wraps at 65535 to prevent overflow');
log('='.repeat(80));
