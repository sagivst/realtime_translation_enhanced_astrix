...
export class AudioRecorder {
  ...
  capture(frame, ctx, tap) {
    console.log(`[AudioRecorder.capture] Called: trace=${ctx?.trace_id}, station=${ctx?.station_key}, tap=${tap}, frame_type=${frame?.constructor?.name}, frame_length=${frame?.length}`);
    if (!ctx?.trace_id || !ctx?.station_key) {
      console.log(`[AudioRecorder.capture] Missing context - trace_id=${ctx?.trace_id}, station_key=${ctx?.station_key}`);
    }
    ...
    console.log(`[AudioRecorder.capture] Frame converted: pcm_length=${pcm.length}`);
  ...
  _finalizeAndEnqueueSegment(ctx, tap, st) {
    ...
    console.log(`[AudioRecorder] Enqueueing segment: trace=${meta.trace_id}, station=${meta.station_key}, tap=${meta.tap}, bucket_ts=${meta.bucket_ts_ms}, samples=${merged.length}`);
