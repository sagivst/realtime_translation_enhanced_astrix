...
export class Aggregator {
  ...
  addSample(s) {
    ...
    if (stationSet) {
      ...
      console.log("[KNOB-DEBUG] Added station", s.station_key, "to bucket", new Date(bucketTs).toISOString());
    }
    ...
    if (!agg) {
      agg = {
        ...
        station_group: s.station_group || null,
        layer: s.layer || null,
  ...
  async _saveKnobSnapshots(bucketTs, stationKeys) {
    ...
    console.log("[KNOB-DEBUG] _saveKnobSnapshots called with:", {bucketTs: new Date(bucketTs).toISOString(), stationCount: stationKeys ? stationKeys.size : 0, hasResolver: !!this.knobsResolver, hasBridge: !!this.databaseBridge});
    if (!this.knobsResolver || !this.databaseBridge) return;
    console.log("[KNOB-DEBUG] Checking conditions - knobsResolver:", !!this.knobsResolver, "databaseBridge:", !!this.databaseBridge);
    ...
    for (const station_key of stationKeys) {
      try {
        ...
        // Extract trace_id from bucket data for this station
        let trace_id = 'GLOBAL'; // Default fallback
        const bucketData = this.buckets.get(bucketTs);
        if (bucketData) {
          // Look for any series key that matches this station
          for (const [seriesKey, aggState] of bucketData) {
            // seriesKey format: trace_id|station_key|tap|metric_key
            const parts = seriesKey.split('|');
            if (parts[1] === station_key) {
              trace_id = parts[0]; // Extract the trace_id
              break;
            }
          }
        }
        ...
        this.databaseBridge.saveKnobSnapshot({
          config_version: 1, // Default config version (will be updated by BucketScheduler when knobs are applied)
          trace_id: trace_id, // Now using actual trace_id from metrics
          station_key,
          bucket_ts_ms: bucketTs,
  ...
  _flushClosedBuckets(nowMs) {
    ...
    for (const bucketTs of toFlush) {
      ...
      console.log("[KNOB-DEBUG] Flush: bucketTs=", new Date(bucketTs).toISOString(), "stationKeys=", stationKeys ? Array.from(stationKeys) : "undefined", "size=", stationKeys?.size);
