...
async function checkUDPSocket(port, description) {
    try {
        ...
        const { stdout: sttPid } = await execAsync('pgrep -f STTTSserver || true');
...
async function checkPostgreSQL() {
    try {
        ...
        const { stdout: pgPid } = await execAsync('pgrep -f "postgres.*-D" || pgrep postgres || true');
...
async function checkPgrepProcess(processName) {
    try {
        const { stdout: pidOutput } = await execAsync(`pgrep -f ${processName} || true`);
