...
sttttServerSocket.on('stationMetrics', (data) => {
  ...
  if (stations[normalizedStationId]) {
    ...
    // Directly merge all incoming metrics into station.metrics
    // This supports the dotted-notation metrics from UniversalCollector
    Object.assign(station.metrics, metrics);

    // Update last update timestamp
    station.metrics.lastUpdate = timestamp;

    // Use our real collected metrics directly (already have all 75 parameters)
    // station.metrics now contains the dotted-notation parameters from UniversalCollector
    const fullMetrics = station.metrics;

    // Broadcast to all connected dashboard clients
    console.log(`[Emit] Broadcasting to dashboard - stationId: ${normalizedStationId}, metrics count: ${Object.keys(fullMetrics).length}`);
    console.log(`[Emit] Metrics:`, Object.keys(fullMetrics));
    ...
    // Also emit kebab-case event for dashboard compatibility
    io.emit('station-update', {
      stationId: normalizedStationId,
      metrics: fullMetrics
    });
    console.log(`[Emit] ✓ Emitted station-update for ${normalizedStationId}`);
  }
});
