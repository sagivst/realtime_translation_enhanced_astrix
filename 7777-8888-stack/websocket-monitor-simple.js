...
// Statistics
const stats = {
  '7777': { packetsReceived: 0, bytesReceived: 0, lastActivity: null },
  '8888': { packetsReceived: 0, bytesReceived: 0, lastActivity: null }
};
...
async function main() {
  ...
  setInterval(() => {
    ...
    if (clientCount > 0) {
      const summary = {
        clients: clientCount,
        '7777': `${stats['7777'].packetsReceived} packets (${(stats['7777'].bytesReceived / 1024).toFixed(2)} KB)`,
        '8888': `${stats['8888'].packetsReceived} packets (${(stats['8888'].bytesReceived / 1024).toFixed(2)} KB)`
      };
