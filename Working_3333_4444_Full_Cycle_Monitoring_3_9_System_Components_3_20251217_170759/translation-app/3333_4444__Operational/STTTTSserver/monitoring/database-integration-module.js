...
const dbConfig = {
  ...
  // No password for local postgres with trust auth
  max: 20,
...
class DatabaseIntegration {
  constructor() {
    // Use trust authentication for local postgres
  const config = {...dbConfig};
  if (config.user === "postgres" && config.host === "localhost") {
    delete config.password; // Remove password for local postgres
  }
  ...
  async initializeDatabase() {
    try {
      ...
      console.error("Database not available - running without database integration");
    }
  }
